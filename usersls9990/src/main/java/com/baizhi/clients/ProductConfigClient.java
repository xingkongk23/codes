package com.baizhi.clients;

import com.baizhi.entity.StudentEntity;
import com.baizhi.fallback.ProductConfigClientFallBack;
import org.springframework.cloud.openfeign.FeignClient;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestParam;

import java.util.Map;

@FeignClient(value ="productslsconfig",fallback = ProductConfigClientFallBack.class)
public interface ProductConfigClient {

    //展示商品信息
    @GetMapping("/productConfig/showMsg")
    String showMsg();

    //测试openFeign传递参数
    @GetMapping("/productConfig/findById")
    Map<String,Object> findById(@RequestParam("id")String id);

    //测试openFeign传递对象
    @GetMapping("/productConfig/findStudent")
    Map<String,Object> findStudent(@RequestBody StudentEntity po);

    //测试服务熔断
    @GetMapping("/productConfig/break")
    public String testBreak(@RequestParam Integer id);
}
